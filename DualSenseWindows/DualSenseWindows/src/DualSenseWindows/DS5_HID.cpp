...
DWORD DS5W::getHIDInputReport(HANDLE device, LPOVERLAPPED ol, UCHAR* buffer, size_t length)
{
	// Start an Overlapped I/O read
	ResetEvent(ol->hEvent);
	BOOL res = ReadFile(device, buffer, length, NULL, ol);
...
DWORD DS5W::setHIDOutputReport(HANDLE device, LPOVERLAPPED ol, UCHAR* buffer, size_t length)
{
	// Start an overlapped write
	ResetEvent(ol->hEvent);
	BOOL res = WriteFile(device, buffer, length, NULL, ol);
...
DWORD DS5W::getHIDFeatureReport(HANDLE device, LPOVERLAPPED ol, UCHAR* buffer, size_t length)
{
	...
	// start read request
	ResetEvent(ol->hEvent);
	BOOL res = DeviceIoControl(device,
